...
const Category = props => {
    ...
    if (sortByData)
        variables.simiProductSort = sortByData
    ...
    return (
        <Simiquery query={cateQuery} variables={variables}>
            {({ loading, error, data }) => {
                ...
                Identify.storeDataToStoreage(Identify.SESSION_STOREAGE, 'last_viewed_cate_id', data.category.id);
                Identify.storeDataToStoreage(Identify.SESSION_STOREAGE, 'last_viewed_cate_path_array', pathArray);
